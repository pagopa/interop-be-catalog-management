...
import it.pagopa.pdnd.interop.uservice.catalogmanagement.model.{Attributes, EService, Problem}
import spray.json._
...
class EServiceApiMarshallerImpl extends EServiceApiMarshaller with SprayJsonSupport with DefaultJsonProtocol {
  ...
  override implicit def fromStringUnmarshallerAttributes: FromStringUnmarshaller[Attributes] =
    Unmarshaller.strict(txt => txt.parseJson.convertTo[Attributes])

  override implicit def fromStringUnmarshallerStringList: FromStringUnmarshaller[Seq[String]] =
    Unmarshaller.strict(txt => txt.parseJson.convertTo[Seq[String]])
}

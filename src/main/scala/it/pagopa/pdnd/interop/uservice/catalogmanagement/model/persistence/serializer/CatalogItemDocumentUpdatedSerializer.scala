...
import it.pagopa.pdnd.interop.uservice.catalogmanagement.model.persistence.CatalogItemDocumentUpdated
...
class CatalogItemDocumentUpdatedSerializer extends SerializerWithStringManifest {
  ...
  final val CatalogItemDocumentUpdatedManifest: String = classOf[CatalogItemDocumentUpdated].getName

  override def toBinary(o: AnyRef): Array[Byte] = o match {
    case event: CatalogItemDocumentUpdated =>
      serialize(event, CatalogItemDocumentUpdatedManifest, currentVersion)
  }

  @SuppressWarnings(Array("org.wartremover.warts.Throw"))
  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef = manifest.split('|').toList match {
    case CatalogItemDocumentUpdatedManifest :: `version1` :: Nil =>

...
import it.pagopa.interop.catalogmanagement.model.persistence.{CatalogEventsSerde, Event}
...
final case class CatalogNotificationProjection(
...
final case class NotificationProjectionHandler(queueWriter: QueueWriter)(implicit ec: ExecutionContext)
  ...
  override def process(envelope: EventEnvelope[Event]): DBIO[Done] = {
    ...
    val kind: String                            = CatalogEventsSerde.getKind(envelope.event)

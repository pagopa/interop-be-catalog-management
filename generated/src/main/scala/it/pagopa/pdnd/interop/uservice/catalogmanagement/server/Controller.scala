package it.pagopa.pdnd.interop.uservice.catalogmanagement.server
...
class Controller(eService: EServiceApi, validationExceptionToRoute: Option[ValidationException => Route] = None)(implicit system: ActorSystem) {
...
path("pdnd-interop-uservice-catalog-management" / "build-info") {
get {
complete(HttpEntity(ContentTypes.`application/json`, buildinfo.BuildInfo.toJson))
}
}

lazy val routes: Route = getBuildInfo ~ pathPrefix("pdnd-interop-uservice-catalog-management" / interfaceVersion) {

...
class ApiInvoker(formats: Formats)(implicit system: ActorSystem) extends CustomContentTypes with Json4sSupport {
  import it.pagopa.pdnd.interop.uservice.catalogmanagement.client.invoker.ApiInvoker._
  import it.pagopa.pdnd.interop.uservice.catalogmanagement.client.invoker.ParametersMap._
  ...
  val CompressionFilter: HttpMessage ⇒ Boolean = (msg: HttpMessage) =>
    Seq(
      ...
      .map(f => f(msg))
      .forall(identity)
  private def addAuthentication(credentialsSeq: Seq[Credentials]) = { request: HttpRequest =>
  ...
  private def headers(headers: Map[String, Any]) =
    headers.asFormattedParams.map { case (name, value) => RawHeader(name, value.toString) }.toSeq
  private def bodyPart(name: String, value: Any): BodyPart = {
    value match {
      case f: File =>
        BodyPart.fromFile(name, ContentType(MediaTypes.`application/octet-stream`), f, f.length().toInt)
      case v: String =>
  ...
  private def formDataContent(request: ApiRequest[_]) = {
    ...
    if (params.isEmpty)
      ...
      Some(normalizedContentType(request.contentType).mediaType match {
...
//            FormData(params.mapValues(_.toString))
  ...
  private def bodyContent(request: ApiRequest[_]): Option[Any] = request.bodyParam
  ...
  private def createRequest(uri: Uri, request: ApiRequest[_]): HttpRequest = {
    ...
    addAuthentication(request.credentials)(httpRequest.withHeaders(headers(request.headerParams)))
  }
  def makeQuery(r: ApiRequest[_]): Query = {
    r.credentials
      ...
      .foldRight[Query](Uri.Query.Empty) { case ((name, value), acc) =>
        acc.+:(name, value)
      }
  }
